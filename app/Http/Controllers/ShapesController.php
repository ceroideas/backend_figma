...
class ShapesController extends Controller
{
    ...
    public function recursiveCalculate($newNode,$j)
    {
        ...
        // csvData
        if (!isset($this->csvData[$j])) {$this->csvData[$j] = [$newNode->name => 0];}
        $this->csvData[$j] = array_merge($this->csvData[$j], [$newNode->name => 0]);
        // print_r($formula);
        return $formula;
    }
    public function generateSimulation(Request $r)
    {
        ...
        for ($j=0; $j < $this->simulationNumber; $j++) { 
            ...
            // csvData
            if (!isset($this->csvData[$j])) {$this->csvData[$j] = [$tierCero->name => 0];}
            $this->csvData[$j] = array_merge($this->csvData[$j], [$tierCero->name => 0]);
