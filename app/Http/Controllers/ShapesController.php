...
class ShapesController extends Controller
{
    ...
    public function recursiveCalculate($newNode)
    {
        ...
        for ($i = 0; $i < count($newNode->formula); $i++) {
            ...
            if (gettype($nodeId) == 'integer') {
                ...
                if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                if ($node->type == 1) {
                        ...
                        switch ($node->distribution_shape[0]['name']) {
                            case 'Uniforme':
                                $randomNumber = $this->uniformOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $randomNumber;
                                break;
                            case 'Triangular':
                                $triangularNumber = $this->triangularOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $triangularNumber;
                                break;
                            case 'Binominal':
                                $binomialNumber = $this->binomialOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $binomialNumber;
                                break;
                            case 'Lognormal':
                                $lognormalNumber = $this->lognormalOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $lognormalNumber;
                                break;
                            case 'Geometric':
                                $geometricNumber = $this->geometricalOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $geometricNumber;
                                break;
                            case 'Weibull':
                                $weibullNumber = $this->weibullOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $weibullNumber;
                                break;
                            case 'Beta':
                                $betaNumber = $this->betaOperation(
                                  ...
                                  $formula[] = '(' . $betaNumber . ')';
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $betaNumber;
                                break;
                            case 'Hypergeometric':
                                $hypergeometricNumber = $this->hypergeometricOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $hypergeometricNumber;
                                break;
                            case 'Poisson':
                                $poissonNumber = $this->poissonOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $poissonNumber;
                                break;
                            case 'Normal':
                                $randomNumberNormal = $this->normalOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $randomNumberNormal;
                                break;
                            case 'Exponencial':
                                $randomNumberExponential = $this->exponentialOperation(
                                  ...
                                  if (!isset($csvData[$i])) {$csvData[$i][$node->name] = [];}
                                  $csvData[$i][$node->name] = $randomNumberExponential;
                                break;
                            default:
                                break;
                        }
    ...
    public function generateSimulation(Request $r)
    {
        ...
        for ($j=0; $j < $this->simulationNumber; $j++) { 
            ...
            for ($i = 0; $i < count($tierCero->formula); $i++) {
                ...
                if (gettype($nodeId) == 'integer') {
                    ...
                    $node = $this->projectNodes->firstWhere('id', $nodeId);
                    if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                    $csvData[$j]["id"] = $simulationId;
                    if ($node->type == 1) {
                        if (!in_array($node->id, $this->nodesActive)) {
                          $value =
                            $node->unite == null || $node->unite == ""
                            ...
                            switch ($node->distribution_shape[0]['name']) {
                                case 'Uniforme':
                                    $randomNumber = $this->uniformOperation(
                                      ...
                                      $formula[] = '(' . $randomNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $randomNumber;
                                    break;
                                case 'Triangular':
                                    $triangularNumber = $this->triangularOperation(
                                        $node->distribution_shape[0]['min'],
                                      ...
                                      $formula[] = '(' . $triangularNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $triangularNumber;
                                    break;
                                case 'Binominal':
                                    $binomialNumber = $this->binomialOperation(
                                        $node->distribution_shape[0]['trials'],
                                      ...
                                      $formula[] = '(' . $binomialNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $binomialNumber;
                                    break;
                                case 'Lognormal':
                                    $lognormalNumber = $this->lognormalOperation(
                                        $node->distribution_shape[0]['mean'],
                                      ...
                                      $formula[] = '(' . $lognormalNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $lognormalNumber;
                                    break;
                                case 'Geometric':
                                    $geometricNumber = $this->geometricalOperation(
                                        $node->distribution_shape[0]['probability'],
                                      ...
                                      $formula[] = '(' . $geometricNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $geometricNumber;
                                    break;
                                case 'Weibull':
                                    $weibullNumber = $this->weibullOperation(
                                        $node->distribution_shape[0]['form'],
                                      ...
                                      $formula[] = '(' . $weibullNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $weibullNumber;
                                    break;
                                case 'Beta':
                                    $betaNumber = $this->betaOperation(
                                        $node->distribution_shape[0]['alpha'],
                                      ...
                                      $formula[] = '(' . $betaNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $betaNumber;
                                    break;
                                case 'Hypergeometric':
                                    $hypergeometricNumber = $this->hypergeometricOperation(
                                        $node->distribution_shape[0]['population'],
                                      ...
                                      $formula[] = '(' . $hypergeometricNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $hypergeometricNumber;
                                    break;
                                case 'Poisson':
                                    $poissonNumber = $this->poissonOperation(
                                        $node->distribution_shape[0]['lamda'],
                                      ...
                                      $formula[] = '(' . $poissonNumber . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $poissonNumber;
                                    break;
                                case 'Normal':
                                    $randomNumberNormal = $this->normalOperation(
                                        $node->distribution_shape[0]['mean'],
                                      ...
                                      $formula[] = '(' . $randomNumberNormal . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $randomNumberNormal;
                                    break;
                                case 'Exponencial':
                                    $randomNumberExponential = $this->exponentialOperation(
                                        $node->distribution_shape[0]['rate'],
                                      ...
                                      $formula[] = '(' . $randomNumberExponential . ')';
                                      if (!isset($csvData[$j])) {$csvData[$j][$node->name] = [];}
                                      $csvData[$j][$node->name] = $randomNumberExponential;
                                    break;
                                default:
                                    break;
                            }
        ...
        return ["arrayToSee"=>$arrayToSee,"csvData"=>$csvData];
    }
